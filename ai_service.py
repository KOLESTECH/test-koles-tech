...
TARIFFS = {
    Tariff.MINI.value: {
        ...
        "channels_limit": 1,
        "daily_posts_limit": 1,
        "ai_requests_limit": 2,  # Ограничение по запросам
        "ai_copies_limit": 1,
    ...
    Tariff.STANDARD.value: {
        ...
        "channels_limit": 3,
        "daily_posts_limit": 5,
        "ai_requests_limit": 15,  # Ограничение по запросам
        "ai_copies_limit": 5,
    ...
    Tariff.PRO.value: {
        ...
        "ai_requests_limit": 30,  # Ограничение по запросам
        "ai_copies_limit": 10,
...
class AdvancedAISessionManager:
    ...
    def get_session(self, user_id: int) -> Dict:
        ...
        if user_id not in self.sessions:
            ...
            self.sessions[user_id] = {
                ...
                'request_count': 0,
                'total_requests': 0,
                'ai_requests_used': 0,  # Новый счетчик для запросов
                'copies_used': 0,
                'last_reset': now.date(),
    ...
    def increment_ai_requests(self, user_id: int) -> int:
        """Увеличивает счетчик AI запросов"""
        session = self.get_session(user_id)
        session['ai_requests_used'] += 1
        return session['ai_requests_used']
    ...
    def get_user_stats(self, user_id: int) -> Dict:
        ...
        session = self.get_session(user_id)
        return {
            'ai_requests_used': session['ai_requests_used'],
    ...
    def get_system_stats(self) -> Dict:
        ...
        total_ai_requests = sum(s['ai_requests_used'] for s in self.sessions.values())
